// Shared intent contract: the response schema, the prompt rules and the validator.
// Lives apart from intent.parser so the voice path (voice.parser.ts) can use it even when
// tests replace the text parser with a mock.
...
export const UNKNOWN_INTENT: UserIntent = { intent: "UNKNOWN" };
...
export function buildIntentRules(context?: IntentContext): string {
  ...
  return `You ONLY classify the message. You never decide whether anything should be approved.
